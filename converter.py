...
def process_csv(df: pd.DataFrame, store_type: str = "競り1（1号店・送料別）", image_settings: dict = None) -> tuple[pd.DataFrame, int]:
    ...
    result_df['即決価格'] = result_df['開始価格'] + 10
    result_df['個数'] = 10
    result_df['期間'] = 7
    result_df['終了時間'] = 0
    ...
    for p_code, group in result_df.groupby('親品番', sort=False, dropna=False):
        ...
        if not is_title_over_50:
            for seq, (idx, row) in enumerate(group.iterrows(), start=1):
                ...
                # 保証案内の追加
                html = html + "<B>●保証について</B><BR>保証内容はご購入頂いた商品のみとなります。<BR>当社では初期不良、商品保証期間で対応が異なります。<BR><B><A HREF=https://store.shopping.yahoo.co.jp/solltd/solpage01.html TARGET=new>コチラ</A></B>をご一読ください。 <BR> <BR>"
